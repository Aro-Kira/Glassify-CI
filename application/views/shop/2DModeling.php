...
<main class="container">
    ...
    <section class="product-details">
        ...
        <div class="price-box" id="price-box">
            ...
            <span class="price-value">₱3,100.00</span>
        </div>
        ...
        <div id="summary-wrapper" class="hidden-step">
            ...
            <div class="summary-actions">
                <button class="cart-btn" id="add-to-cart-btn" data-product-id='<?= $product_id ?>'>
    ...
    <div class="inner-content">
        ...
        <div class="testimonial-slider">
            ...
            <div class="testimonial-text-box">
                <p class="quote">"Highly recommending this shop! Very smooth and fast transaction. Despite unfortunate
...
<script>
// Require login before Add to Cart or Buy Now actions.
(function(){
    // Determine logged-in state on server-side (CodeIgniter session)
    const isLoggedIn = <?= ($this->session->userdata('user_id') || $this->session->userdata('logged_in')) ? 'true' : 'false'; ?>;

    // Capture clicks on the buttons before other handlers run.
    document.addEventListener('click', function(e){
        const btn = e.target.closest('#add-to-cart-btn, #buy-now-btn');
        if (!btn) return; // not our buttons

        if (isLoggedIn) return; // user signed in -> allow normal flow

        // Not signed in: prevent other handlers and prompt to login
        e.preventDefault();
        e.stopPropagation();

        // Build a redirect path relative to site root so `site_url($redirect)` on the server
        // doesn't double-prefix the base URL. Example result: redirect=2DModeling?id=2
        const baseSiteUrl = '<?= site_url('') ?>';
        let redirectPath = window.location.href;
        if (redirectPath.indexOf(baseSiteUrl) === 0) {
            redirectPath = redirectPath.substring(baseSiteUrl.length);
        } else {
            // fallback: use pathname without leading slash plus query string
